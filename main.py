...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")
        await self.highrise.walk_to(Position(15.5 , 0.0 , 3.0 , "FrontRight"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")   
        await self.highrise.chat(f"😍BIENVENUE😍 {user.username} ✨DANS LA SALLE DE GRAB DE LAURA✨FAITE COMME CHEZ VOUS🤭✨.Pour les danses utilise de 1 à 95")
